...
export default function ExpensesByCategory({
  ...
  const { categories, fetchCategories, createCategory, updateCategory } =
    useCategoryStore();
  ...
  const onSubmitBudget = async (data: UpdateBudgetFormData) => {
    ...
    try {
      ...
      const message =
        error.response?.data?.message || "Erro ao atualizar limite";
      toast.error(message);
    } finally {
      setIsSubmitting(false);
    }
  ...
  const typeOptions = Object.entries(TRANSACTION_TYPE_LABELS).map(
    ([value, label]) => ({
  ...
  const categoriesWithoutExpenses = categoryExpenses.filter(
    cat => cat.total === 0
  );
  ...
  return (
    <>
          ...
          {categories.length === 0 ? (
            ...
            >
              ...
              {categoriesWithExpenses.map(category => {
                ...
                const isNearBudget =
                  hasBudget && budgetPercentage > 80 && budgetPercentage <= 100;
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            handleOpenBudgetModal(category.id, category.budget)
                          }
                          title="Editar limite"
                        >
      ...
      {/* Quick Add Category Modal - 3 Steps */}
      <QuickAddCategory
      ...
      >
        ...
        >
          ...
          <p className="text-sm text-muted-foreground">
            Defina quanto você quer gastar no máximo nesta categoria. A barra de
            progresso mostrará seu consumo em relação ao limite.
          </p>
